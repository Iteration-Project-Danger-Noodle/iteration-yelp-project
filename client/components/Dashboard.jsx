...
function Dashboard({ user, username, setUser, zipcode }) {
  ...
  //   } else setFetchedData(dummyData.businesses);
  // }, [username]);
  useEffect(() => {
    if (username) {
      axios.post('http://localhost:3000/yelp/search', { term: '', location: zipcode })
      .then((response) => {
        const rawData = response.data.businesses;
        console.log(rawData);
        setFetchedData(rawData);
